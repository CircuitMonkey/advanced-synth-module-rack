...
void OLED::begin() {
  ...
  //  }
  //Serial.begin(115200);
  Wire.setSDA(SDA);  // setup SDA and SCL pins
  Wire.setSCL(SCL);
  ...
  adaSplashAddOns();
  display.display();
  delay(1400); // Pause for 1.4 seconds
...
void OLED::loop() {
  ...
  //testanimate(ada_logo_bmp, LOGO_WIDTH, LOGO_HEIGHT); // Animate bitmaps
}

void OLED::adaSplashAddOns() {
  display.setTextSize(1); // Draw 2X-scale text
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(35, 0);
  display.println(F("OLED Library"));
}
...
void OLED::testdrawbitmap(void) {
  ...
  display.drawBitmap(
    ...
    ada_logo_bmp, LOGO_WIDTH, LOGO_HEIGHT, 1);
  display.display();
  delay(1000);
}
